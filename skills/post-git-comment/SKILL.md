...
3. `author-reviewable-text`에 산출물 종류가 이슈 또는 PR·MR 코멘트임을 밝히고 Markdown 형식과 사용자 제공 원문 유지 같은 필수 형식, 확정된 내용인 확인된 사실, provider의 길이 제한과 대상별 옵션을 전달한다. 반환된 게시 초안을 사용한다.
4. 대상 provider, 이슈 또는 PR·MR 번호, 게시 본문 전문을 보여주고 사용자 승인을 받는다.
5. 승인 전에는 코멘트를 게시하거나 기존 코멘트를 수정하지 않는다.
