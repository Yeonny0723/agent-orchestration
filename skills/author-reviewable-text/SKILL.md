...
- 선택 사항인 voice profile

확인되지 않은 내용을 추정해 사실로 만들지 않는다. 필수 입력이 모호하면 초안을 쓰기 전에 호출자에게 부족한 근거를 요청한다.

이 skill이 voice profile을 직접 확인한다. 먼저 `$AGENT_ORCHESTRATION_HOME/voice-profile.md`를 확인하고 없으면 `~/.agent-orchestration/voice-profile.md`를 확인한다.
...
4. 두 선택형 skill이 없거나 호출 중 오류가 나면 해당 단계를 건너뛰며 전체 작성을 차단하지 않는다. 누락이나 실패마다 정확한 skill 이름과 실패 원인을 호출자에게 보고하고, 대체 skill을 찾거나 설치하지 않는다.
5. 필수 절, 확인된 사실, 숫자, 파일명, 명령어, 선택지, 차이, 판단 기준, 인용문과 길이 제한이 그대로 보존됐는지 확인한다.
6. 우선순위에 다시 대조한 뒤 최종 초안을 한 번만 호출자에게 반환한다.
