...
사용자 대면 질문과 산출물을 만들기 전에 `$AGENT_ORCHESTRATION_HOME/voice-profile.md`를 확인하고, 환경 변수가 없으면 `~/.agent-orchestration/voice-profile.md`를 확인한다. 파일이 있으면 문체에만 적용한다. 없으면 `capture-authoring-voice`로 설정할 수 있다고 한 번만 안내하고 기본 한국어 문체로 계속하며 차단하지 않는다.
